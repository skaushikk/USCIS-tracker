...
def app():
    ...
    with st.beta_expander('Case Details', expanded=False):
        ...
        st.write('Application Type:', f'Form I-{formno}')
    ...
    with st.beta_expander('Range Analysis', expanded=False):
        ...
        st.subheader(f"{approved.values[0]} of the similar cases are APPROVED. {'{:.0%}'.format(pending.sum())} of cases are still PENDING.")